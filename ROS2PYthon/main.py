...
# --- ADAUGA ACEASTA FUNCTIE PENTRU UNITY ---
def UnityHandle():
    global x, y, z, nozzle_st, nozzle_angle, move_sleep
    
    # Verificăm dacă sunt date pe Serial (USB)
    if select.select([sys.stdin], [], [], 0)[0]:
        line = sys.stdin.readline().strip()
        
        try:
            # Exemplu format comanda din Unity: "X,Y,Z,Angle,Nozzle"
            # Ex: "10,150,50,0,1"
            parts = line.split(',')
            if len(parts) == 5:
                ...
                # 1. Control Poziție (Inverse Kinematics)
                if arm.set_position((nx, ny, nz), 100):
                    x, y, z = nx, ny, nz
                
                # 2. Control Unghi Ventuză
                nozzle_angle = n_angle
                nozzle.set_angle(nozzle_angle)

                # 3. Control Pompă (Ventuza)
                if n_pump == 1: nozzle.on()
                else: nozzle.off()
                
                print("Unity ACK: Moved to", nx, ny, nz)
        except:
            pass
